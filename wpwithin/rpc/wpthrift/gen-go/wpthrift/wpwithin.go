...
func (p *WPWithinClient) InitConsumer(scheme string, hostname string, port int32, urlPrefix string, clientID string, hceCard *wpthrift_types.HCECard, pspConfig map[string]string) (err error) {
	if err = p.sendInitConsumer(scheme, hostname, port, urlPrefix, clientID, hceCard, pspConfig); err != nil {
		return
	}
...
func (p *WPWithinClient) InitProducer(pspConfig map[string]string) (err error) {
	if err = p.sendInitProducer(pspConfig); err != nil {
		return
	}
...
func (p *WPWithinClient) CloseRPCAgent() (err error) {
	if err = p.sendCloseRPCAgent(); err != nil {
		return
	}
...
func NewWPWithinCallbackProcessor(handler WPWithinCallback) *WPWithinCallbackProcessor {
	...
	self110 := &WPWithinCallbackProcessor{handler: handler, processorMap: make(map[string]thrift.TProcessorFunction)}
	self110.processorMap["beginServiceDelivery"] = &wPWithinCallbackProcessorBeginServiceDelivery{handler: handler}
	self110.processorMap["endServiceDelivery"] = &wPWithinCallbackProcessorEndServiceDelivery{handler: handler}
	return self110
}
...
func (p *WPWithinCallbackProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	iprot.Skip(thrift.STRUCT)
	iprot.ReadMessageEnd()
	x111 := thrift.NewTApplicationException(thrift.UNKNOWN_METHOD, "Unknown function "+name)
	oprot.WriteMessageBegin(name, thrift.EXCEPTION, seqId)
	x111.Write(oprot)
	oprot.WriteMessageEnd()
	oprot.Flush()
